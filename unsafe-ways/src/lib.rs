...
mod tests {
    ...
    fn test_split_into_equal_parts() {
        ...
        assert_eq!(split_into_equal_parts(&mut v, 3), &[&[1, 2], &[3, 4], &[5, 6]]);
    }

    #[test]
    fn test_str_to_bytes_horribly_unsafe() {
        let bytes: &[u8] = unsafe {
            std::mem::transmute::<&str, &[u8]>("Going off the menu") 
        }; 

        assert_eq!(bytes, &[71, 111, 105, 110, 103, 32, 111, 102, 102, 32, 116, 104, 101, 32, 109, 101, 110, 117]);        
    }
}
